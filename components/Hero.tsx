...
type DropItem = {
  id: string; name: string; tech: string; price: number; 
  status: string; img: string; gif?: string; description: string;
};
...
export function Hero() {
  ...
  // ── FILTRO: APENAS OS 3 PRIMEIROS PARA O CARROSSEL ──
  const carouselProducts = allDrops.slice(0, 3); 
  
  const staticProducts = {
    esquerda: allDrops[6], // Camiseta 007
    direita: allDrops[7]   // Regata 008
  };
    ...
    <>
      <section id="drops" className="grid grid-cols-1 md:grid-cols-[260px_1fr_260px] min-h-[96vh] border-b-2 border-ink relative overflow-hidden bg-bg">
        ...
        <div className="hidden md:flex bg-bg-panel border-r-2 border-ink flex-col items-center justify-between p-6 relative z-30">
          ...
          <div 
            onClick={() => setSelectedProduct(staticProducts.esquerda)}
          ...
          >
            ...
            <div className="font-sans font-black uppercase tracking-tighter text-base text-ink leading-none mb-1">{staticProducts.esquerda.name}</div>
          </div>
        </div>

        {/* ALTAR CENTRAL (CARROSSEL TRAVADO NOS 3) */}
        <div className="flex flex-col items-center justify-center py-10 px-0 md:px-8 relative overflow-hidden bg-bg w-full">
          ...
          <div className="relative w-full h-80 md:h-100 flex items-center justify-center z-20 mb-8">
            ...
            <button onClick={nextSlide} className="absolute right-2 md:right-4 z-40 w-10 h-10 md:w-12 md:h-12 border-2 border-ink bg-bg flex items-center justify-center font-sans font-black text-xl text-ink hover:bg-gold hover:text-ink transition-colors cursor-pointer shadow-[2px_2px_0_var(--color-ink)]">
              &gt;
            </button>
            ...
            {carouselProducts.map((item, index) => {
              ...
              if (diff === 0) transformClass = "translate-x-0 opacity-100 scale-100 z-30 shadow-[12px_12px_0_var(--color-ink)] cursor-pointer animate-float"; 
        ...
        <div className="hidden md:flex bg-bg-panel border-l-2 border-ink flex-col items-center justify-between p-6 relative z-30 gap-6">
          ...
          <div 
            onClick={() => setSelectedProduct(staticProducts.direita)}
          ...
          >
            ...
            <div className="font-sans font-black uppercase tracking-tighter text-base text-ink leading-none mb-1">{staticProducts.direita.name}</div>
          </div>
        </div>
      </section>
