...
export function DropsGrid() {
  ...
  const drops: DropItem[] = [
    ...
    { 
      ...
      description: "Um patrimônio da cultura brasileira transformado em relíquia. Esta camisa de futebol foi customizada à mão com o bordado 'MADE IN BELO JARDIM' nas costas e reconstituída nos modelos de um crooped. Peça única 1/1." 
    },
    { 
      id: "005", 
    ...
    <>
      <section id="colecoes" className="flex flex-col border-t-2 border-ink">
        <div className="flex justify-between items-center py-6 px-8 bg-bg-panel border-b-[1.5px] border-ink">
          <span className="font-space text-xs font-bold tracking-[0.2em] uppercase text-ink/70">— Vitrine de Relíquias</span>
          <span className="font-space text-[10px] font-bold tracking-[0.15em] uppercase text-ink">{drops.length} Peças no total</span>
        </div>
            ...
            <div key={item.id} className="border-b-[1.5px] border-ink md:border-r-[1.5px] flex flex-col hover:bg-[#e8dfc8] transition-colors group">
              ...
              >
                ...
                <Image 
                  ...
                  alt={item.name} 
                  fill 
                  className={`object-cover absolute top-0 left-0 transition-all duration-700 z-10 ${item.gif ? 'group-hover:opacity-0' : 'group-hover:scale-110'} ${item.status === 'sold_out' ? 'grayscale opacity-80' : ''}`} 
                />
                <span className="absolute top-4 left-4 font-space text-[10px] font-bold text-ink bg-bg px-2 py-1 z-20 border-[1.5px] border-ink">#{item.id}</span>
                {item.status === 'coming_soon' && <span className="absolute top-4 right-4 font-space text-[9px] font-bold tracking-widest uppercase bg-gold text-ink px-2 py-1 z-20">Em breve</span>}
                {item.status === 'sold_out' && <span className="absolute top-4 right-4 font-space text-[9px] font-bold tracking-widest uppercase bg-ink text-bg px-2 py-1 z-20">Esgotado</span>}
              </div>
              ...
              <div className="p-6 flex flex-col justify-between flex-1">
                <div onClick={() => openModal(item)} className="cursor-pointer">
                  ...
                  <h3 className="font-sans font-black uppercase tracking-tighter text-2xl text-ink leading-tight mb-4 group-hover:text-gold transition-colors">{item.name}</h3>
                </div>
                <div className="flex items-center justify-between mt-auto">
                  ...
                  {item.status === 'available' ? (
                    <button 
                      ...
                      className="font-sans font-black text-xs uppercase tracking-widest text-gold bg-ink px-4 py-2 hover:bg-gold hover:text-ink transition-all cursor-pointer border-2 border-ink active:scale-95"
                    >+ Sacola</button>
                  ) : (
      ...
      {/* ── MODAL COM GALERIA E CURSOR POINTER ── */}
      {selectedProduct && (
